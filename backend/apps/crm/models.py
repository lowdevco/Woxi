...
class Contact(models.Model):
    ...
    tags = models.ManyToManyField(
        Tag, through='ContactTag', related_name='contacts')
    created_at = models.DateTimeField(auto_now_add=True)
...
class ContactCustomValue(models.Model):
    ...
    contact = models.ForeignKey(
        Contact, on_delete=models.CASCADE, related_name='custom_values')
...
class Conversation(models.Model):
    ...
    assigned_agent = models.ForeignKey(
        User, on_delete=models.SET_NULL, blank=True, null=True, related_name='assigned_conversations')
...
class Message(models.Model):
    ...
    content_type = models.CharField(
        max_length=20, choices=CONTENT_CHOICES, default='text')
    content_text = models.TextField(blank=True, null=True)
    media_url = models.URLField(max_length=1024, blank=True, null=True)
    template_name = models.CharField(max_length=255, blank=True, null=True)
    message_id = models.CharField(
        max_length=255, blank=True, null=True)  # WhatsApp Meta Message ID
    status = models.CharField(
...
class WhatsAppConfig(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name='whatsapp_config')
...
class MessageTemplate(models.Model):
    ...
    header_type = models.CharField(
        max_length=20, choices=HEADER_CHOICES, blank=True, null=True)
...
class Deal(models.Model):
    ...
    stage = models.ForeignKey(
        PipelineStage, on_delete=models.CASCADE, related_name='deals')
    contact = models.ForeignKey(
        Contact, on_delete=models.CASCADE, related_name='deals')
    conversation = models.ForeignKey(
        Conversation, on_delete=models.SET_NULL, blank=True, null=True, related_name='deals')
    ...
    status = models.CharField(
        max_length=50, default='active')  # active, won, lost
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
