...
import { FiSend as Send, FiVolume2 as Campaign, FiVolume2 as CampaignOutlined, FiStar as Star } from 'react-icons/fi';
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800 }}>Broadcast Campaigns</Typography>
          <Typography variant="subtitle1" color="text.secondary">Send bulk messages using Meta-approved WhatsApp templates.</Typography>
        </Box>
        <Button 
          variant="contained" 
          startIcon={<Campaign />} 
      ...
      <Paper sx={{ border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
        {loading ? (
          ...
          <Box sx={{ py: 6, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
            ...
            <Typography color="text.secondary" sx={{ fontWeight: 500 }}>No broadcasts sent yet.</Typography>
          </Box>
        ) : (
          <TableContainer>
            <Table>
              <TableHead sx={{ bgcolor: 'background.paper' }}>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 700 }}>Delivered / Read</TableCell>
                  <TableCell sx={{ fontWeight: 700 }} align="right">Date Created</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {broadcasts.map((b) => (
                  <TableRow key={b.id} hover>
                    ...
                    <TableCell>
                      <Stack direction="row" spacing={1}>
                        <Chip label={`Delivered: ${b.delivered_count || 0}`} size="small" variant="outlined" />
                        <Chip label={`Read: ${b.read_count || 0}`} size="small" variant="outlined" color="primary" />
                      </Stack>
                    </TableCell>
      ...
      <Dialog open={open} onClose={handleCloseDialog} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ fontWeight: 700 }}>New Broadcast Campaign</DialogTitle>
        <form onSubmit={handleCreateBroadcast}>
          <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: 1 }}>
            ...
            <FormControl fullWidth required>
              ...
              >
                ...
                {templates.length === 0 && (
                  <MenuItem disabled>No approved templates found in Meta account.</MenuItem>
                )}
              </Select>
            </FormControl>
          </DialogContent>
          <DialogActions sx={{ px: 3, pb: 3 }}>
            ...
            <Button type="submit" variant="contained" endIcon={<Send />}>Launch</Button>
          </DialogActions>
        </form>
      </Dialog>
    </Box>
