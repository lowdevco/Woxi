...
import { 
  Box, Grid, Paper, List, ListItem, ListItemAvatar, ListItemText, 
...
export default function Inbox() {
  ...
  const handleSelectConversation = async (conv) => {
    ...
    try {
      const res = await api.get(`/crm/messages/by_conversation/?conversation_id=${conv.id}`);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const res = await api.post(`/crm/conversations/${activeConv.id}/add_message/`, payload);
  ...
  return (
    <Box sx={{ height: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' }}>
      <Paper sx={{ flexGrow: 1, display: 'flex', overflow: 'hidden', border: '1px solid', borderColor: 'divider' }}>
        <Grid container sx={{ height: '100%' }}>
          
          {/* Sidebar Conversations List */}
          <Grid item xs={12} sm={4} sx={{ borderRight: '1px solid', borderColor: 'divider', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ p: 2 }}>
              <TextField
                fullWidth
                size="small"
            ...
            <Box sx={{ flexGrow: 1, overflowY: 'auto' }}>
              {loadingChats ? (
                ...
                <Typography align="center" color="text.secondary" sx={{ py: 4, fontWeight: 500 }}>
                        ...
                        >
                          <Badge color="primary" badgeContent={conv.unread_count}>
                            <Avatar sx={{ bgcolor: 'primary.light' }}>
                              {conv.contact_detail?.name?.substring(0, 2).toUpperCase() || <AccountCircle />}
                            </Avatar>
                          </Badge>
                          
                          <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                              <Typography variant="body1" sx={{ fontWeight: conv.unread_count > 0 ? 800 : 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {conv.contact_detail?.name || conv.contact_detail?.phone}
                              </Typography>
          ...
          <Grid item xs={12} sm={8} sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#f1f5f9' }}>
            {activeConv ? (
              <>
                {/* Chat Panel Header */}
                <Box sx={{ px: 3, py: 2, bgcolor: 'background.paper', borderBottom: '1px solid', borderColor: 'divider', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Avatar sx={{ bgcolor: 'primary.main' }}>
                      {activeConv.contact_detail?.name?.substring(0, 2).toUpperCase() || <AccountCircle />}
                    </Avatar>
                    <Box>
                      <Typography variant="body1" sx={{ fontWeight: 700 }}>
                        {activeConv.contact_detail?.name || activeConv.contact_detail?.phone}
                      </Typography>
                      <Typography variant="caption" color="text.secondary">
                ...
                <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 3, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {loadingMessages ? (
                    ...
                    <Box sx={{ flexGrow: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Typography color="text.secondary" sx={{ fontWeight: 500 }}>No messages yet. Send one below!</Typography>
                    </Box>
                  ) : (
                    messages.map((msg) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <Typography variant="body2">{msg.content_text}</Typography>
                            <Typography 
                              variant="caption" 
                              align="right" 
                              display="block" 
              ...
              <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: 3 }}>
                <Avatar sx={{ bgcolor: 'primary.light', width: 64, height: 64, mb: 2 }}>
                  <ChatIcon sx={{ fontSize: 32, color: 'primary.contrastText' }} />
                </Avatar>
                <Typography variant="h6" sx={{ fontWeight: 700 }}>No Chat Selected</Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>Select a contact from the list on the left to start messaging in real-time.</Typography>
              </Box>
            )}
          </Grid>
