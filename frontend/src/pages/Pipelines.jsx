...
export default function Pipelines() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (pipesRes.data.length > 0) {
        ...
        const newPipe = await api.post('/crm/pipelines/', { name: 'Sales Pipeline' });
        
        // Seed standard pipeline stages
        const stage1 = await api.post('/crm/stages/', { pipeline: newPipe.data.id, name: 'Lead', position: 1, color: '#3b82f6' });
        const stage2 = await api.post('/crm/stages/', { pipeline: newPipe.data.id, name: 'Contacted', position: 2, color: '#f59e0b' });
        const stage3 = await api.post('/crm/stages/', { pipeline: newPipe.data.id, name: 'Proposal Sent', position: 3, color: '#8b5cf6' });
        const stage4 = await api.post('/crm/stages/', { pipeline: newPipe.data.id, name: 'Negotiation', position: 4, color: '#ec4899' });
        const stage5 = await api.post('/crm/stages/', { pipeline: newPipe.data.id, name: 'Won', position: 5, color: '#10b981' });
  ...
  const onDrop = async (e, targetStageId) => {
    ...
    setPipeline(prev => {
      const updatedDeals = prev.deals?.map(d => d.id === dealId ? { ...d, stage: targetStageId } : d);
  ...
  return (
    <Box sx={{ height: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800 }}>Deals Pipeline</Typography>
          <Typography variant="subtitle1" color="text.secondary">Kanban pipeline of active client sales and deals.</Typography>
        </Box>
      </Box>

      {/* Kanban Board Container */}
      <Box sx={{ flexGrow: 1, overflowX: 'auto', display: 'flex', gap: 3, pb: 2 }}>
        {pipeline?.stages?.map((stage) => {
          // Filter deals belonging to this stage
          const stageDeals = pipeline.deals?.filter(deal => deal.stage === stage.id) || [];
          const stageTotalValue = stageDeals.reduce((sum, deal) => sum + parseFloat(deal.value || 0), 0);
            ...
            >
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Typography variant="body1" sx={{ fontWeight: 700 }}>{stage.name}</Typography>
                  <Chip label={stageDeals.length} size="small" sx={{ height: 18, fontSize: 10, fontWeight: 700 }} />
                </Box>
                <IconButton size="small" onClick={() => handleOpenDialog(stage.id)}>
                  <Add fontSize="small" />
                </IconButton>
              </Box>
                  ...
                  >
                    <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                        <Avatar sx={{ width: 20, height: 20, bgcolor: 'primary.light', fontSize: 10 }}>
                          {deal.contact_detail?.name?.substring(0, 1).toUpperCase() || <AccountCircle sx={{ fontSize: 12 }} />}
                        </Avatar>
                        <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600 }}>
                          {deal.contact_detail?.name || deal.contact_detail?.phone}
                        </Typography>
                      </Box>
